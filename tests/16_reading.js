...
const LANGS = ['en', 'es', 'pt', 'fr', 'de', 'it', 'ru', 'ja', 'ko', 'tl', 'sv', 'ceb', 'ar', 'he', 'uk', 'pl', 'zh-Hans', 'zh-Hant', 'yue-Hant', 'vi', 'hu', 'hi', 'id', 'ro'];
...
(async () => {
  ...
  section('jw.org wtlocale codes');
  {
    const VERIFIED = {
      ...
      pl: 'P',
      // jw.org splits Chinese three ways and so do we; each probed live.
      'zh-Hans': 'CHS', 'zh-Hant': 'CH', 'yue-Hant': 'CHC',
      // `VI` serves Russian and `V` serves Slovak — both silently.
      vi: 'VT',
      // `HU` serves English and `M` serves Romanian.
      hu: 'H',
      // `H1` serves Hungarian and `IN` serves Indonesian.
      hi: 'HI',
      // `ID` — the language's own ISO code — serves English, `I` serves
      // Italian, and `INS` serves Indonesian *Sign* Language.
      id: 'IN',
      // `RO` — the ISO code — serves English, `R` serves Western Armenian,
      // and `RU` serves Rundi (Russian is `U`).
      ro: 'M',
    };
    const src = fs.readFileSync(REPO + '/js/reading.js', 'utf8');
