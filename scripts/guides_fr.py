...
GUIDES_FR["import-markdown-notes-jw-library"] = {
  "title": "Comment importer des notes Markdown dans JW Library",
  "h1": "Comment importer des notes Markdown dans JW Library",
  "description": "Transformez des fichiers .md d'Obsidian, de Notion ou de n'importe quel éditeur Markdown en véritables notes JW Library, placées sur le bon verset — gratuit, privé, dans votre navigateur.",
  "intro": [
   "JW Library n'offre aucun moyen d'importer du texte. Vous pouvez écrire des notes dans l'application, mais une note rédigée ailleurs — dans Obsidian, dans Notion, dans un simple fichier texte — n'a aucun chemin vers votre bibliothèque. On finit par tout retaper, ou par renoncer et garder deux jeux de notes d'étude qui ne se rejoignent jamais.",
   "JW Sync comble ce manque. Chargez une sauvegarde dans l'Explorateur d'étude, appuyez sur « Importer du Markdown », et vos fichiers .md deviennent de vraies notes à l'intérieur : titre, date et étiquettes conservés et — quand le fichier indique le passage concerné — rattachées à ce verset, exactement comme si vous aviez écrit la note dans l'application en le lisant.",
   "Cela fonctionne dans les deux sens. Les notes exportées depuis ce site portent leur livre, leur chapitre et leur verset : elles retournent donc précisément d'où elles venaient. Les notes écrites ailleurs ne portent en général rien de tel, et c'est pourquoi cette page explique les une ou deux lignes qui les placent — et ce qui se passe quand un fichier ne les a pas.",
  ],
  "steps": [
   ("Créez une sauvegarde dans JW Library",
    "Ouvrez JW Library, allez dans Étude personnelle, touchez le menu à trois points, choisissez Sauvegarde et restauration, puis Créer une sauvegarde. Vous obtenez un fichier .jwlibrary — la bibliothèque à laquelle vos notes seront ajoutées."),
   ("Ouvrez l'Explorateur d'étude",
    "Allez sur jwsync.org, ouvrez l'Explorateur d'étude et chargez ce fichier .jwlibrary. Tout se passe dans votre navigateur ; le fichier n'est jamais envoyé nulle part."),
   ("Appuyez sur « Importer du Markdown »",
    "Le bouton se trouve à côté d'« Exporter en Markdown ». Choisissez autant de fichiers .md que vous voulez, ou un .zip qui les contient — y compris le .zip que ce site produit à l'export."),
   ("Lisez le récapitulatif avant que quoi que ce soit soit écrit",
    "On vous indique combien de notes se placeront sur un verset, combien seront ajoutées comme notes libres, et combien figurent déjà dans votre sauvegarde et seront ignorées. Tout ce que le site a dû interpréter est listé ligne par ligne, avec une case à cocher, pour que vous décidiez au lieu de le découvrir ensuite."),
   ("Exportez et restaurez",
    "Appuyez sur « Exporter .jwlibrary », puis restaurez ce fichier dans JW Library via Sauvegarde et restauration → Restaurer. Vos notes importées font désormais partie de votre bibliothèque sur cet appareil."),
  ],
  "sections": [
   ("Le fichier le plus court qui fonctionne",
    "Un fichier Markdown n'a besoin de rien pour être importé : un fichier ne contenant que du texte devient une note, dont le titre vient du premier titre interne ou, à défaut, du nom du fichier. Tout le reste sert à dire où va la note. Un fichier complet ressemble à ceci :\n\n---\ntitle: La coupe pour laquelle il a prié\ntags: [étude, gethsémané]\npublication: Matthew 26:39\n---\n\nSa prière montre la soumission, non la réticence.\n\nLe bloc entre les deux lignes de tirets s'appelle le front matter. Les lignes en dessous sont la note elle-même."),
   ("Les champs qui sont lus",
    "Cinq choses seulement sont lues, et chacune accepte plusieurs noms pour vous éviter d'en retenir un précis. **title** (ou name, heading) devient le titre de la note. **tags** (ou tag, keywords, categories, labels, topics) deviennent de vraies étiquettes JW Library, créées si elles n'existent pas encore. **date** (ou created, modified) fixe la date de la note ; toute valeur contenant AAAA-MM-JJ est comprise. **publication** (ou pub, reference, ref, scripture, citation, source, passage) est l'endroit où vous écrivez le passage. **book**, **chapter** et **verse** (ou ch, v, vs, verses) font la même chose dans des champs séparés. Tout autre champ que vous gardez pour votre usage — auteur, statut, ce que votre éditeur ajoute — est ignoré plutôt que traité comme une erreur."),
   ("Trois façons de rattacher une note à un verset",
    "Écrivez la référence sur une ligne : `publication: Matthew 26:39`. Ou découpez-la : `book: Matthew`, `chapter: 26`, `verse: 39`. Ou faites les deux — c'est ce que fait l'export de ce site, pour que le fichier se lise bien pour un humain et s'analyse exactement pour une machine. Les trois donnent le même résultat : utilisez ce qui convient à votre éditeur."),
   ("Jusqu'où la référence peut être approximative",
    "Les noms de livres sont reconnus largement. Les noms complets fonctionnent, et aussi les abréviations que les gens tapent réellement : Matt, Matt., Mt, 1 Cor, 1Cor, I Corinthians, Second Timothy, Psalm aussi bien que Psalms, Song of Songs aussi bien que Song of Solomon. Le séparateur entre chapitre et verset peut être deux-points, un point ou la lettre v — `John 3:16`, `John 3.16` et `1 Cor 13 v4` se lisent de la même façon. Les espaces sont facultatifs, donc `1Cor13:4` convient aussi.\n\nLe front matter est tout aussi tolérant. Les noms de champs peuvent porter des majuscules. L'espace après les deux-points peut manquer. Les espaces et tabulations superflus sont ignorés, de même que les guillemets autour des valeurs et un commentaire final avec #. Les fins de ligne Windows ne posent pas de problème. Vous pouvez même omettre les lignes --- et commencer le fichier directement par les champs, à condition que la toute première ligne soit l'un des noms reconnus — cette règle existe pour qu'une note débutant par une phrase ordinaire contenant deux-points ne perde pas son premier paragraphe."),
   ("Les étiquettes, écrites comme vous voulez",
    "Toutes ces formes produisent les deux mêmes étiquettes : `tags: [étude, grec]`, `tags: étude, grec`, `tags: étude; grec`, `tags: #étude #grec`, ou une liste sur ses propres lignes sous `tags:`, chaque entrée commençant par un tiret. Les étiquettes qui existent déjà dans votre bibliothèque sont réutilisées et non dupliquées : la note importée rejoint donc l'étiquette avec laquelle vous filtrez déjà."),
   ("Ce qui se passe quand quelque chose n'est pas clair",
    "Rien d'incertain n'est décidé à votre place. Si un nom de livre ressemble à un vrai sans être exact — `Mathew`, `Ecclesiates` —, la note n'est pas placée automatiquement. Elle apparaît dans le récapitulatif, sur sa propre ligne, avec ce que le site en a compris : « lu comme Matthew 26:39 », et une case à cocher. Décochez-la et elle est laissée de côté ; laissez-la cochée et elle va là où la ligne l'indique.\n\nSi le livre n'est pas reconnu du tout, la ligne le dit et la note est ajoutée comme note libre — jamais forcée sur une supposition. Un livre numéroté garde son numéro : `1 Jonh` ne peut être corrigé qu'en 1 Jean, jamais en 2 Jean."),
   ("Ce qu'il ne fera délibérément pas",
    "Une plage de versets rattache la note à son premier verset : `Matthew 26:39-41` place la note au verset 39, parce qu'une note JW Library s'attache à un point du texte et non à un intervalle. Une référence avec un chapitre mais sans verset — `Matthew 26` — rattache la note à ce chapitre. Un fichier qui nomme une publication plutôt qu'un passage — `The Watchtower—2023 No. 4` — est importé comme note libre ordinaire sans aucune question, puisque c'est exactement ce que l'export de ce site écrit pour une note prise dans un article."),
   ("La mise en forme qui survit au voyage",
    "Les paragraphes, les retours à la ligne, le **gras**, l'*italique* et les listes à puces arrivent sous forme de vraie mise en forme dans JW Library. Un titre en tête de fichier sert de titre à la note au lieu d'être répété à l'intérieur. Tout ce que Markdown sait exprimer et que les notes JW Library ne savent pas — tableaux, images, liens, blocs de code — est réduit à son texte : aucun mot n'est perdu, même quand la disposition ne peut pas être reprise."),
   ("Où les notes atterrissent réellement, et pourquoi c'est sûr",
    "Pour rattacher une note à un verset, JW Library a besoin d'identifiants internes de ce chapitre, propres à votre bibliothèque. JW Sync ne les invente jamais. Il réutilise l'emplacement que votre propre sauvegarde possède déjà pour ce chapitre, ou copie ces identifiants depuis un autre emplacement biblique du même fichier. Si une sauvegarde ne contient aucune note ni surlignage dans la Bible, il n'y a rien à copier : les notes sont alors ajoutées comme notes libres plutôt que rattachées à quelque chose que l'application pourrait ne pas reconnaître. C'est un choix délibéré : une note qui atterrit silencieusement au mauvais endroit est pire qu'une note qui arrive franchement détachée."),
   ("Importer deux fois les mêmes fichiers",
    "Une note est considérée comme déjà présente quand son titre, son texte et son emplacement correspondent à quelque chose dans la sauvegarde : réimporter le même export ne double donc rien. Le récapitulatif vous dit combien seront ignorées pour cette raison avant que vous validiez. Tout ce qu'un import ajoute constitue une seule étape d'annulation, et les notes importées reçoivent l'étiquette « Imported » pour que vous puissiez ensuite les retrouver, les filtrer ou les supprimer en bloc."),
  ],
  "faq": [
   ("Suis-je obligé d'utiliser les lignes --- ?",
    "Non. Elles rendent le fichier sans ambiguïté et tous les éditeurs Markdown les comprennent, mais vous pouvez aussi commencer le fichier directement par les champs — `title: …` en première ligne — ou vous passer du front matter et laisser la note prendre son titre du premier titre interne."),
   ("Et si j'écris mal un nom de livre ?",
    "Le site devine généralement ce que vous vouliez dire, mais il n'agit pas sur cette seule base. La note apparaît dans le récapitulatif avec la lecture retenue et une case à cocher, pour que vous confirmiez avant que quoi que ce soit soit écrit."),
   ("Puis-je importer un dossier Obsidian entier ?",
    "Sélectionnez autant de fichiers .md que vous voulez en une fois, ou compressez le dossier et choisissez le .zip. Les fichiers qu'il contient sont lus comme les fichiers isolés."),
   ("L'import va-t-il écraser mes notes existantes ?",
    "Non. L'import ne fait qu'ajouter. Vos notes, surlignages, signets et étiquettes actuels ne sont pas touchés, et le fichier chargé n'est jamais modifié — vous téléchargez un nouveau .jwlibrary à la fin."),
   ("Mes notes sont-elles envoyées quelque part ?",
    "Non. Tout fonctionne dans votre navigateur, comme le reste du site. Ni votre sauvegarde ni vos fichiers Markdown ne quittent votre appareil."),
   ("Dans quelles langues puis-je écrire la référence ?",
    "Pour l'instant les noms de livres sont reconnus en anglais, ce que l'export écrit également. Si vous rédigez vos notes dans une autre langue, utilisez les champs `book`, `chapter` et `verse` avec le nom anglais du livre, ou importez les notes détachées et placez-les ensuite à la main."),
   ("Puis-je ressortir mes notes ?",
    "Oui — « Exporter en Markdown », sur le même écran, écrit chaque note dans un fichier .md avec les mêmes champs : les notes peuvent donc sortir de JW Library et y revenir sans perdre leur place."),
  ],
}

GUIDES_FR["write-markdown-notes-for-jw-library"] = {
  "title": "Comment écrire un fichier Markdown qui s'importe dans JW Library",
  "h1": "Comment écrire un fichier Markdown qui s'importe dans JW Library",
  "description": "Écrivez une note dans le Bloc-notes, TextEdit ou Obsidian et faites-la arriver sur le bon verset dans JW Library. Les lignes exactes à taper, et tout ce que vous pouvez omettre.",
  "intro": [
   "JW Sync sait lire des fichiers Markdown pour les faire entrer dans une sauvegarde JW Library, ce qui soulève la seule question qui compte vraiment une fois qu'on sait que la fonction existe : à quoi le fichier doit-il ressembler ? Cette page y répond du côté de celui qui écrit — non pas ce que l'importateur accepte, mais ce que vous tapez réellement.",
   "En bref, une note est un fichier texte. Créez le fichier, écrivez la note, enregistrez-le en le terminant par .md, et il s'importera. Tout le reste de cette page porte sur une seule ligne facultative : celle qui indique à quel passage la note appartient, pour qu'elle apparaisse dans JW Library pendant que vous lisez ce verset au lieu de rester isolée parmi vos autres notes.",
   "Rien de tout cela n'exige un éditeur Markdown, et rien n'exige que vous appreniez le Markdown. Le Bloc-notes sous Windows et TextEdit sur Mac suffisent. Si vous tenez déjà vos notes d'étude dans Obsidian ou Notion, ces fichiers sont déjà du Markdown, et il y a une section pour chacun plus bas.",
  ],
  "steps": [
   ("Ouvrez n'importe quel éditeur de texte brut",
    "Le Bloc-notes sous Windows, TextEdit sur Mac, ou l'application Markdown que vous utilisez déjà — Obsidian, Typora, iA Writer, Joplin. La seule chose à éviter est un traitement de texte : Word et Google Docs enregistrent de la mise en forme plutôt que du texte brut, et ni l'un ni l'autre ne produit un fichier que l'importateur puisse lire."),
   ("Tapez la note",
    "Mettez le titre sur la première ligne, précédé d'un #, laissez une ligne vide, puis écrivez la note en dessous. C'est déjà un fichier complet et valide. Si vous préférez ne pas écrire de ligne de titre, c'est le nom du fichier qui sera utilisé."),
   ("Enregistrez-le en le terminant par .md",
    "Appelez-le comme vous voulez : ma note.md. Sous Windows, le Bloc-notes enregistre un .txt tant que vous n'avez pas changé « Type » en « Tous les fichiers » — c'est l'erreur la plus fréquente. Sur Mac, choisissez Format puis Convertir au format texte avant d'enregistrer."),
   ("Ajoutez la ligne qui place la note sur un verset",
    "Tout en haut du fichier, au-dessus de tout le reste, écrivez une ligne de trois tirets, puis publication: Matthew 26:39, puis de nouveau trois tirets. La note est désormais rattachée à ce verset. Sans cette ligne, la note s'importe quand même : elle arrive simplement non rattachée."),
   ("Importez-la dans une sauvegarde",
    "Dans JW Library, allez dans Étude personnelle, ouvrez le menu à trois points, choisissez Sauvegarde et restauration et créez une sauvegarde. Chargez ce fichier sur jwsync.org, dans l'Explorateur d'étude, appuyez sur « Importer du Markdown », choisissez vos fichiers .md et lisez le récapitulatif affiché avant que quoi que ce soit soit écrit. Appuyez ensuite sur « Exporter .jwlibrary » et restaurez ce fichier dans JW Library."),
  ],
  "sections": [
   ("Le fichier le plus court qui fonctionne",
    "Un fichier ne contenant que du texte est déjà une note valide. Le voici en entier :\n\n```\n# La coupe pour laquelle il a prié\n\nSa prière montre la soumission, non la réticence.\n```\n\nLa ligne qui commence par # devient le titre de la note, et tout ce qui se trouve en dessous devient la note. Ce fichier s'importe sans protester. Il arrive simplement comme une note isolée, parmi vos notes plutôt que rattachée à un passage."),
   ("Créer le fichier sous Windows",
    "Ouvrez le Bloc-notes, tapez la note, puis choisissez Fichier et Enregistrer sous. Avant d'enregistrer, changez « Type » de « Documents texte » à « Tous les fichiers » — sans cette étape, le Bloc-notes ajoute discrètement .txt et vous vous retrouvez avec ma note.md.txt, que l'importateur ne verra pas. Tapez ensuite le nom en le terminant par .md. Windows peut vous avertir que changer l'extension risque de rendre le fichier inutilisable ; cet avertissement est le signe que vous avez bien fait."),
   ("Créer le fichier sur un Mac",
    "Ouvrez TextEdit et choisissez Format puis Convertir au format texte avant de taper quoi que ce soit. TextEdit démarre en mode texte enrichi et enregistre du .rtf, qui n'est pas du tout un fichier texte. Une fois en mode texte brut, choisissez Fichier puis Enregistrer, et terminez le nom par .md. Si macOS propose d'ajouter le .txt, refusez."),
   ("Créer le fichier sur un téléphone ou une tablette",
    "N'importe quelle application capable d'enregistrer ou d'exporter du Markdown convient — Obsidian, Bear, iA Writer, Joplin, Markor sur Android. Les applications Notes intégrées à l'iPhone et à Android ne savent généralement pas enregistrer un fichier .md, donc sur téléphone la voie habituelle passe par l'une de ces applications, ou tout simplement par un ordinateur. Au moment d'importer, vous pouvez sélectionner autant de fichiers que vous voulez d'un coup, et un dossier entier peut être compressé et fourni sous forme d'un seul .zip."),
   ("La seule ligne qui place une note sur un verset",
    "Tout ce qui indique à l'importateur où va la note se met dans un petit bloc tout en haut du fichier, entre deux lignes de trois tirets. Cela s'appelle le front matter, et pour la plupart des notes il tient sur une ligne :\n\n```\n---\npublication: Matthew 26:39\n---\n\n# La coupe pour laquelle il a prié\n\nSa prière montre la soumission, non la réticence.\n```\n\nCette note est maintenant rattachée à Matthieu 26:39. Elle apparaît dans JW Library pendant que vous lisez ce verset, exactement comme si vous l'aviez écrite là depuis l'application."),
   ("Écrire la référence pour qu'elle soit comprise",
    "Écrivez le nom du livre en anglais, même quand la note est dans une autre langue : Matthew, et non Matthieu. Les abréviations que les gens tapent réellement sont comprises elles aussi — Matt, Matt., Mt, 1 Cor, 1Cor, I Corinthians, Second Timothy. Entre le chapitre et le verset, vous pouvez utiliser deux-points, un point ou la lettre v : John 3:16, John 3.16 et 1 Cor 13 v4 veulent tous dire la même chose, et les espaces sont facultatifs, si bien que 1Cor13:4 convient aussi. Un intervalle comme Matthew 26:39-41 place la note sur le premier verset, le 39, car une note JW Library se rattache à un point du texte et non à un passage entier."),
   ("Le fichier complet, avec un titre, une date et des étiquettes",
    "Trois lignes de plus sont disponibles si vous les voulez, et toutes sont facultatives :\n\n```\n---\ntitle: La coupe pour laquelle il a prié\ndate: 2026-08-15\ntags: [étude, gethsémané]\npublication: Matthew 26:39\n---\n\nSa prière montre la soumission, non la réticence.\n```\n\ntitle fait la même chose que le titre précédé d'un #, donc l'un des deux suffit. date fixe la date de la note au lieu de laisser le jour de l'importation. tags deviennent de vraies étiquettes JW Library sur lesquelles vous pouvez filtrer, et une étiquette que vous avez déjà est réutilisée plutôt que dupliquée. La ligne tags peut aussi s'écrire étude, gethsémané sans crochets, ou #étude #gethsémané, ou encore sous forme de liste à tirets sur des lignes séparées en dessous — les quatre formes se lisent de la même façon."),
   ("Une note par fichier",
    "Chaque fichier .md devient une note. Il n'y a pas moyen de mettre plusieurs notes dans un même fichier et de les faire séparer, car rien dans un fichier Markdown ne marque de façon fiable où une note se termine et où la suivante commence — un titre à mi-chemin peut être une deuxième note comme il peut être un sous-titre de la première, et se tromper disperserait ce que vous avez écrit. Écrivez donc un fichier par note, puis sélectionnez-les tous d'un coup au moment d'importer, ou compressez le dossier et choisissez le .zip."),
   ("La mise en forme qui arrive intacte",
    "Les paragraphes, les retours à la ligne, le gras, l'italique et les listes à puces arrivent dans JW Library comme véritable mise en forme. Les titres situés après le premier deviennent des paragraphes ordinaires. Tout ce que le Markdown sait exprimer et qu'une note JW Library ne sait pas — tableaux, images, liens, blocs de code — est réduit à son texte, de sorte que les mots survivent même là où la mise en page ne peut pas suivre."),
   ("Si vous écrivez déjà dans Obsidian",
    "Les fichiers Obsidian sont déjà du Markdown, et son panneau Propriétés écrit exactement le front matter décrit ici : ajoutez une propriété nommée publication avec la valeur Matthew 26:39 et la note atterrira là. La propriété tags propre à Obsidian est lue comme tags. Les liens entre doubles crochets et les notes intégrées sont réduits à leur texte, puisqu'une note JW Library n'a nulle part où les faire pointer. Vous pouvez sélectionner tous les .md d'un dossier en une seule importation, ou compresser le dossier et fournir le .zip."),
   ("Si vous écrivez déjà dans Notion",
    "Notion sait exporter du Markdown : ouvrez le menu de la page, choisissez Exporter et sélectionnez « Markdown & CSV ». Ce que vous obtenez est un .zip, que l'importateur accepte tel quel. Notion écrit le titre de la page sous forme de titre et ses propres propriétés sous forme de lignes ordinaires en dessous, et les champs que l'importateur ne reconnaît pas sont ignorés plutôt que traités comme des erreurs — donc, pour placer une note sur un verset, ajoutez vous-même une ligne publication en haut du fichier exporté."),
   ("Ce que vous n'avez pas besoin de réussir",
    "Les majuscules dans les noms de champs n'ont aucune importance. L'espace après les deux-points peut manquer. Les espaces et tabulations en trop sont ignorés, tout comme les guillemets autour d'une valeur et un commentaire écrit après un #. Les fins de ligne Windows ne posent pas de problème. Vous pouvez même omettre complètement les lignes --- et commencer le fichier directement par les champs, à condition que la toute première ligne en soit un. Et si vous écrivez mal le nom d'un livre — Mathew —, l'importateur vous dit qu'il a lu cela comme Matthew 26:39 et attend que vous cochiez la case, au lieu de trancher tout seul."),
   ("Ce que l'importation ne fera pas à votre bibliothèque",
    "Importer ne fait qu'ajouter. Les notes, surlignages, signets et étiquettes déjà présents dans votre sauvegarde ne sont pas touchés, et le fichier que vous chargez n'est jamais modifié — à la fin vous téléchargez un nouveau .jwlibrary et c'est celui-là que vous restaurez. Une note dont le titre, le texte et l'emplacement correspondent à quelque chose qui se trouve déjà dans la sauvegarde est ignorée, donc importer deux fois les mêmes fichiers ne double rien. Tout ce qu'une importation ajoute tient en une seule étape d'Annuler, et les notes importées portent une étiquette Imported, ce qui vous permet ensuite de les retrouver, de filtrer dessus ou de les supprimer en bloc. Rien de tout cela ne quitte votre appareil."),
  ],
  "faq": [
   ("Dois-je d'abord apprendre le Markdown ?",
    "Non. Un fichier de phrases ordinaires est déjà une note valide. Le Markdown n'intervient que si vous voulez du gras, de l'italique ou des puces, et même là il s'agit de deux astérisques autour d'un mot pour le gras et d'un tiret en début de ligne pour une puce."),
   ("Le nom du fichier a-t-il de l'importance ?",
    "Seulement quand le fichier n'a pas de titre à lui. S'il n'y a ni ligne title: ni titre précédé d'un #, le nom du fichier est nettoyé et utilisé à la place : la-coupe-pour-laquelle-il-a-prie.md devient une note appelée « la coupe pour laquelle il a prie ». Sinon, appelez le fichier comme bon vous semble."),
   ("Et si je l'ai enregistré en .txt par erreur ?",
    "Renommez-le pour qu'il se termine par .md et réimportez — rien dans le contenu n'a besoin de changer. Sous Windows, il faudra peut-être d'abord activer les extensions de noms de fichiers dans le menu Affichage de l'Explorateur, sinon la terminaison vous est cachée."),
   ("Puis-je écrire le nom du livre dans ma langue ?",
    "Pas pour l'instant. Les noms de livres sont reconnus en anglais, ce qui est aussi ce qu'écrit l'export Markdown de ce site. Écrivez la note dans la langue que vous voulez ; seule la ligne de référence a besoin du nom anglais. Si vous préférez, laissez la référence de côté et placez la note à la main ensuite."),
   ("Puis-je mettre plusieurs notes dans un même fichier ?",
    "Non — chaque fichier devient une note. Sélectionnez autant de fichiers que vous voulez lors d'une importation, ou compressez le dossier et choisissez le .zip."),
   ("Est-ce que cela va modifier les notes que j'ai déjà ?",
    "Non. Importer ne fait qu'ajouter. Vos notes existantes ne sont pas touchées, et la sauvegarde que vous chargez n'est jamais modifiée : à la fin vous téléchargez un nouveau fichier et vous décidez si vous le restaurez."),
   ("Que se passe-t-il si j'omets la ligne de référence ?",
    "La note s'importe quand même. Elle arrive comme une note isolée dans votre bibliothèque plutôt que rattachée à un passage — ce qui est justement ce que veut d'ordinaire une note sur un discours, une conversation ou une réflexion personnelle."),
  ],
}
