...
# -*- coding: utf-8 -*-
"""
splice_md_import_guide.py — add the Markdown-import guide to every language.

Reads scripts/i18n_data/guide_md_import/<lang>.json and splices the slug into
...
it is not, a language translated twice aborts instead of silently winning, and
re-running is idempotent. It also reports how many of the 24 languages are
present, with the missing ones named, so progress is never guessed at.

Usage:
  python3 scripts/splice_md_import_guide.py           # splice everything present
  python3 scripts/splice_md_import_guide.py --check   # report only
"""
import io
import json
import os
...
DATA = os.path.join(REPO, "scripts", "i18n_data", "guide_md_import")
SLUG = "import-markdown-notes-jw-library"

# guides_i18n.GUIDE_TEXT is the roster; a language absent from it has no guide
# tree at all and must not get an orphan entry.
...
def module_for(lang):
    return os.path.join(REPO, "scripts", "guides_%s.py" % lang.replace("-", "_").lower())


def py_str(s):
    """A Python string literal that survives every quote and newline."""
...
def render(entry, lang, style):
    """The modules come in two shapes and mixing them up is silent corruption.

    Eleven of the twenty-four build their dict with GUIDES_XX["slug"] = {...}
    assignments; the rest are one dict literal. Appending a `"slug": {...},`
    entry to an assignment-style module inserts it *inside the last guide's
    dict* — valid Python, no error, and that guide quietly gains keys it should
    not have. So the shape is detected, never assumed, and the result is
    verified by importing the module afterwards.
    """
    head = (' "%s": {' % SLUG) if style == "literal" else ('%s["%s"] = {' % (var_for(lang), SLUG))
    out = [head]
    out.append('  "title": %s,' % py_str(entry["title"]))
    out.append('  "h1": %s,' % py_str(entry["h1"]))
    out.append('  "description": %s,' % py_str(entry["description"]))
    out.append('  "intro": [')
...
def verify(lang, mod):
    ...
    The whole point: an entry can be written into a file, parse fine and still
    not be a guide. Only the imported dict settles it.
    """
    sys.path.insert(0, os.path.join(REPO, "scripts"))
    ...
    if SLUG not in table:
        sys.exit("ABORT %s: the entry was written but is not in %s — wrong shape"
                 % (mod, var_for(lang)))
    if len(table) != 38:
        sys.exit("ABORT %s: %d guides after splicing, expected 38" % (mod, len(table)))
...
def main():
    ...
    for lang in LANGS:
        ...
        if re.search(r"^%s\[" % re.escape(var), c, re.M):
            style = "assign"                      # GUIDES_XX["slug"] = {...}
            c = c.rstrip("\n") + "\n\n" + render(entry, lang, style)
        elif re.search(r"^%s\s*=\s*\{" % re.escape(var), c, re.M):
            style = "literal"                     # one dict literal
            m = list(re.finditer(r"\n\}\s*$", c))
            if not m:
                sys.exit("ABORT %s: cannot find the end of the guide dict" % mod)
            at = m[-1].start()
            c = c[:at] + "\n" + render(entry, lang, style) + c[at:]
        else:
