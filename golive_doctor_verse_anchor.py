# -*- coding: utf-8 -*-
"""Go live (v2.63.3): copy the Library Doctor duplicate-note safety fix
from beta/index.html into production index.html, and bump the service
worker cache version so PWA users pick it up."""
import io, sys
...
# 5) service worker cache bump so PWA users pick up the new production build
sw = 'service-worker.js'
s = io.open(sw, encoding='utf-8').read()
OLD5 = "CACHE_VERSION = 'jwsync-v104'"
must(s.count(OLD5) == 1, 'CACHE_VERSION anchor not unique: %d' % s.count(OLD5))
s = s.replace(OLD5, "CACHE_VERSION = 'jwsync-v105'", 1)
io.open(sw, 'w', encoding='utf-8').write(s)
print('OK: service-worker.js bumped to jwsync-v105')
